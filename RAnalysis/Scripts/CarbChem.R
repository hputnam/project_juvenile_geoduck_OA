...
#Cumulative_TA_Data.csv
#Daily_Temp_pH_Sal.csv
#Data/pH_Calibration_Files


#############################################################
setwd("~/MyProjects/Geoduck_Epi/project_juvenile_geoduck_OA/RAnalysis/Data") #set working directory
mainDir<-'~/MyProjects/Geoduck_Epi/project_juvenile_geoduck_OA/RAnalysis/' #set main directory
#############################################################


##### DISCRETE pH CALCULATIONS #####
path <-("~/MyProjects/Geoduck_Epi/project_juvenile_geoduck_OA/RAnalysis/Data/pH_Calibration_Files/")
...
daily <- read.csv("SW_Chem_Juvenile_Geoduck.csv", header=TRUE, sep=",", na.strings="NA") #load data with a header, separated by commas, with NA as NA
...
pdf("~/MyProjects/Geoduck_Epi/project_juvenile_geoduck_OA/RAnalysis/Output/Daily_Treatment_Measures.pdf")
par(mfrow=c(3,2))
...
plot(SW.chem$Treatment, SW.chem$Salinity, xlab="Treatment", ylab="Salinity psu", ylim=c(24,30))
dev.off()

pdf("~/MyProjects/Geoduck_Epi/project_juvenile_geoduck_OA/RAnalysis/Output/Daily_Tank_Measures.pdf")
par(mfrow=c(3,2))
...
plot(SW.chem$Sample.ID, SW.chem$Salinity, xlab="Tank", ylab="Salinity psu", ylim=c(24,30))
dev.off()

##### DISCRETE TA CALCULATIONS #####
TA <- read.csv("Cumulative_TA_Output.csv", header=TRUE, sep=",", na.strings="NA")  #read in  TA results
TA <- subset(TA, Type!="Junk" & Type!="CRM")
TA$Sample.ID 
...
SW.chem$Sample.ID <- paste(SW.chem$Measure.Date, SW.chem$Sample.ID, sep='_') #generate new row with concatenated sample id
...
carb.output <- cbind(SW.chem$Measure.Date,  SW.chem$Sample.ID,  SW.chem$Treatment, carb.output) #combine the sample information with the seacarb output
...
write.table(carb.output, "~/MyProjects/Geoduck_Epi/project_juvenile_geoduck_OA/RAnalysis/Output/Seawater_chemistry_table_Output_All.csv", sep=",", row.names = FALSE) #save data
...
pdf("~/MyProjects/Geoduck_Epi/project_juvenile_geoduck_OA/RAnalysis/Output/Water_Chem_withTA.pdf")
par(mfrow=c(3,2))
plot(carb.output$Treatment, carb.output$Temperature, xlab="Treatment", ylab="Temperature°C", ylim=c(11,18))
plot(carb.output$Treatment, carb.output$pH, xlab="Treatment", ylab="pH Total Scale", ylim=c(7.0,8.0))
plot(carb.output$Treatment, carb.output$pCO2, xlab="Treatment", ylab="pCO2 µmol kg-1", ylim=c(350,2000))
plot(carb.output$Treatment, carb.output$Salinity, xlab="Treatment", ylab="Salinity psu", ylim=c(26,29))
plot(carb.output$Treatment, carb.output$TA, xlab="Treatment", ylab="Total Alkalinity µmol kg-1", ylim=c(2600,2900))
